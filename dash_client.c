...
GF_Err gf_dash_download_resource(GF_DashClient *dash, GF_DASHFileIOSession *sess, const char *url, u64 start_range, u64 end_range, u32 persistent_mode, GF_DASH_Group *group)
{
	...
	// while (time < group->inter_request_time)
	// {
	// 	time = gf_sys_clock();
	// }
										  	  ...
										  	  GF_MPD_Representation *rep, Bool go_up_bitrate)
{
    ...
    {
    	actualInterrequestTime = timeNow - group->download_start_time;
    }
  	else
    {
  	...
  	if (group->active_bitrate < rateUp)
    {
  	...
  	else if ((rateUp <= group->active_bitrate)
           && (group->active_bitrate <= rateDown))
    {
    	newIndex = group->active_rep_index;
    }
  	else
    {
    	newIndex = rDown;
...
static s32 dash_do_rate_adaptation_festive(GF_DashClient *dash, GF_DASH_Group *group, GF_DASH_Group *base_group,
										  	  ...
										  	  GF_MPD_Representation *rep, Bool go_up_bitrate)
{
	u32 targetBuf = 30000000;
	double delta = group->segment_duration;
	double alpha = 12.0;
	double throughputThreshold = 0.85;
	int highest_rep_index = gf_list_count(group->adaptation_set->representations) - 1;
	assert(highest_rep_index >= 0);
	...
	if (group->download_segment_index > 1)
	{
		GF_ThroughputHistory *last_th;
		if (gf_list_count(group->throughput_history) > 0)
		{
			last_th = gf_list_get(group->throughput_history,gf_list_count(group->throughput_history)-1);
			last_th->transmissionEnd = group->last_segment_time;
			last_th->transmissionRequested = group->download_start_time;
			last_th->repIndex = group->active_rep_index;
			gf_list_rem(group->throughput_history,gf_list_count(group->throughput_history)-1);
			gf_list_insert(group->throughput_history,last_th,gf_list_count(group->throughput_history)-1);
		}
	}
	double throughputMeasured = ((group->active_bitrate * group->segment_duration) / (group->last_segment_time - group->download_start_time));
	...
	if (group->download_segment_index <= 10)
	{
		group->inter_request_time = 0;
		return 0;
	}
	...
	for (unsigned sd = gf_list_count(group->throughput_history)-1; sd-- > 0; )
    {
    	...
    	throughputATsd = gf_list_get(group->throughput_history,sd);
      	if (throughputATsd->throughput == 0)
        {
        	continue;
        }
      	else
  	...
  	for (u32 i = 0; i < count; i++)
    {
  	...
  	u32 upperBound = targetBuf + delta;
  	u32 bufferNow = 0;
  	u32 randBuf = lowerBound + ( rand () % ( upperBound - (lowerBound) + 1 ) );
 	for (s32 j=0; j<group->nb_cached_segments; j++) {
		bufferNow += group->cached[j].duration;
	}
    ...
    s32 currentRepIndex = group->active_rep_index;
  	...
  	if (currentRepIndex > 0
      	&& group->active_bitrate > thrptEstimation * throughputThreshold)
    {
    ...
    if (currentRepIndex < highest_rep_index && !decisionMade)
    {
      ...
      if (count1 >= 1
          && (double) bitratAtCurrentRep <= thrptEstimation)
        {
          refIndex = currentRepIndex + 1;
          decisionMade = 1;
        }
    ...
    // compute number of bit rate switches in the last 20 seconds
  	...
  	int countIndices = 0;
  	for (unsigned _sd = gf_list_count(group->throughput_history) - 1; _sd-- > 0; )
    {
    	GF_ThroughputHistory *_sd_throughput;
        _sd_throughput = gf_list_get(group->throughput_history,_sd);
        //Chck next transmission requested
    	if (_sd_throughput->transmissionRequested < timeNow)
        {
         	break;
        }
      	else if (currentRepIndex != _sd_throughput->repIndex)
        {
        	u32 found = 0;
        	for(u32 i =0; i<countIndices; i++)
        	{
        		if (foundIndices[i] == currentRepIndex)
        		{
        			found = 1;
        			break;
        		}
          	...
          	if (!found)
            {
          	...
          	numberOfSwitches++;
          	foundIndices[countIndices] = currentRepIndex;
          	count++;
        }
    }
    // compute the scores
    ...
    double scoreEfficiencyCurrent = fabs ((double)bitratAtCurrentRep/ (double)min (thrptEstimation, bitrateAtrefIndex) - 1.0);

  	double scoreEfficiencyRef = fabs ((double)bitrateAtrefIndex
                                        / min (thrptEstimation, (double)bitrateAtrefIndex) - 1.0);
...
static void dash_do_rate_adaptation(GF_DashClient *dash, GF_DASH_Group *group)
{
	...
	*/
	new_index = group->active_rep_index;
	fprintf(stderr, "before calling");
	if (dash->rate_adaptation_algo) {
		fprintf(stderr, "Calling dash rate adaption");
...
{
	fprintf(stderr, "_dash_download_init_segment\n");
	GF_Err e;
	char *base_init_url;
	...
	fprintf(stderr, "Checking for dash algorithm %d",nb_segment_read);
	if (nb_segment_read) {
...
{
	fprintf(stderr, "dash_main_thread_proc");
	GF_Err e;
